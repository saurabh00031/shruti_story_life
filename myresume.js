...
var ct1,ct2,ct3,ct4;
var bulletTime = 0;
var ctext,pythontext,javatext,jstext,dtext;
var birds;
var bubble;
...
function preload(){
	...
	text = game.add.text(w/2, h/2, "Loading into life of \n SHRUTI GUJAR \n 😃😃❤️❤️❤️🎶!!!",{ font:"80px Roboto",fill: '#ffffff' });
	...
	game.load.image('night','https://images.unsplash.com/photo-1590418606746-018840f9cd0f?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTB8fG5pZ2h0fGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=600&q=60');
	...
	game.load.image('frog','ssg1.png');
	game.load.image('sandiskPd','ssg1.png');
	...
	game.load.image('harddrive','ssg1.png');
	...
	game.load.image('google','google.png');
	...
	game.load.image('codelhi','.png');
	game.load.image('pshaktiman','png');
	game.load.image('chai','png');
	game.load.image('football','png');
	game.load.image('gmail','png');
	game.load.image('facebook','png');
	game.load.image('linkedin','png');
	game.load.image('ladder','assets/ladder.png');
	game.load.image('yoga','png');
...
//	game.load.onLoadStart.add(loadStart, this);
  ...
  //  game.stage.scale.startFullScreen();
   ...
   //game.canvas.addEventListener('touchenter', function(){ console.log("You touched here!!");}, false);
	...
	ct5  = game.add.sprite(5700,h,'cocotree');
	...
	javatext = game.add.text(5050,h-320-50,'Maushi 1\n',style_white2);
	pythontext=	game.add.text(5250,h-430-60,'Maushi 2\n',style_white2);
	jstext = game.add.text(5430,h-450-50,'Maushi 3\n',style_white2);
	ctext = game.add.text(5660,h-460-50,'Maushi 4 and\nmama',style_white2);
	...
	game.add.text(5200,h-140,'Skills/tech stack',style_roboto);
	...
	*/
	...
	codetext = game.add.text(6550,h-550,'C    DING...',mustard_style);
   ...
   //Add Explosion object pool
	...
	var style = { font: "20px monospace", fill: "#fff", align: "center" };
	...
	var grass = game.add.tileSprite(0,h-100,platformLength*4,50,'grass');
	...
	var style_ex1 = {font:"32px Arial",fill:"white",align:"center"};
	game.add.text(10900,h-290,"😢😢😢😢😢😢\n (I Love forts,castles,mountains)!!!!",style_ex1);
	...
	game.add.text(11420,h-290,"Holiday Destination is my\n Mama's House 😢😢😢😢😢😢!!!!",style_ex1);
...
//	game.add.text(13300,h-430,"Developed meta-data tracking\nstructures for large storage devices \n using Boost C++ and \n Boost Python Libraries.",style_white);
	...
	var pd = game.add.sprite(14050-100,h-160,'sg4');
	...
	game.add.text(14850,h-450,'My favourite quote: \n EXPECTATIONS \n LEADS TO SORROWS 😢😢😢😢😢😢!',style_white);
	...
	game.add.text(base5+505,h-300,"I hate expectations of people \n but I love peacock 🦚!",style_white);
	...
	game.add.text(base5+1180,h-250,"I participate in events/festivals 😘😘😘 !",style_position);
	...
	game.add.text(base5+2080+50,h-310,"I believe in \n beauty of my dreams !!!",style_position);

	game.add.text(base5+2060,h-305+50,"I hate fake people and dsa little bit 😣😣😣!!!",style_position);
	...
	//Add digital India
	digital_india = game.add.sprite(base5+2700,h-260,'sandiskPd');
	...
	game.add.text(base5+3250,h-470,"Rank under 3600 in \n google kickstart 😳😳😳 	",{font:"30px Arial",fill:"#fff",align:"center"});

	game.add.sprite(base5+3670,h-310,'');
	game.add.text(base5+3650,h-470,"Qualified for \nfinals \n as top 10 team in \n 300 teams in Hack O Furiesta !",{font:"30px Arial",fill:"#fff",align:"center"});
	...
	game.add.text(base6+1240,h-500,"Skills ahet mla patat nahit tr linked in bga \n endorsed (trusted by someone) pn ahet \n😃😃😃!.",style_white);
	game.add.text(base6+1400+20,h-360,"I M RSS student❤️🎶❤️🎶❤️🎶.\n..I serve the nation ❤️🎶❤️🎶❤️🎶❤️🎶 !.",style_white);
	...
	button_spoj = game.add.button(base7+890,h-350,'',openSpoj, this);


	var linkedin_cloud = game.add.sprite(base7+1050-100,h-530,'cloud')
	linkedin_cloud.scale.setTo(1.2,1.2);
	linkedin_cloud.alpha =0.9;
	button_linkedin = game.add.button(base7+990,h-450,'',openLinkedIn, this);
	button_linkedin.scale.setTo(0.8,0.8);
	
	game.add.text(base7+720,h-120,"Sidhiya unko mubaarak ho \njinko manzil tak jana hain !\nMe hauslo se udhan bharti hu \nMuze mera Rasta Khud banana hain! ",style_white);
	
	game.add.button(base7+1260,h-450+20,'',openPublication,this);
	...
	*/
	...
	game.add.text(4300,h-120,"Scored 9.3 pointer marks in \n DY patil university,Kolhapur.\n(Dont consider my very first answer, Its always wrong  😳😳😳 ! )",style_roboto);
	
	//Add bubble
	
	bubble = game.add.sprite(9100,h+450,'psbubble');
	game.add.text(8900,h+500,"Gaavatla la maushicha talaav  😳😳😳 \n  	",style_roboto);
	
	bubble.scale.setTo(0.5,0.5);
	bubble.alpha =0.8;

	inst_text = game.add.text(290,h-50,"(SHRUTI RAMESH GUJAR)...\n I EAT OTHERS BRAIN,BLOOD BUT STILL I HAVE LESS BLOOD 😣😣😣...SAD LIFE",{font:"15px Arial",fill:"#fff"});
	//Add hero 
...
//	hero.body.gravity.y=10000;
	...
	game.add.text(w-120,100,"Good things \ntake Time ️❤️🎶!",{font:"15px Roboto",fill:"white"}).fixedToCamera=true;
...
function update(){
	...
	if(hero.x>platformLength-350){
		...
		t.start();
		t.onComplete.add(function(){game.add.text(platformLength-185,h-370,"Thanks\nfor\n visting\nmy\n short life \njourney\n(Alaytr\ncake\ngheun jaa\nRs.400\nla kilo\n 😳😳😳😳😳\n) !",style_white);},this);
	...
	*/
	...
	if((bug.x - hero.x)<=100){
		...
		if(bug_killed==false){
		game.add.text(bug.x,h-200,"This 5 relatives \n( for me...monkeys) ...\nalways have an eye on me\nno freedom 😣😣😣\nsure but i will \nwork for my freedom ...!!",style_roboto);
		bug_killed =true;
		}
	}
	//Throw onto Ground
...
function loadStart() {
	...
	text.anchor.setTo(0.5,0.5);
	text.setText("Loading into life of \n SHRUTI GUJAR !!! ...");
